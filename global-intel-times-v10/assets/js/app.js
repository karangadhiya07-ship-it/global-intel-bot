...
function renderArticlePage() {
  ...
  root.innerHTML = `<article class="article-detail">
    ...
    <img class="article-hero-image" src="${safeAttr(a.image)}" alt="${escapeHTML(a.title)}" loading="eager" onerror="this.src='${placeholderImage(a.title)}'">
...
function bigCard(a) {
  ...
  return `<article class="hero-card"><a href="./article.html?id=${a.id}"><div class="card-image-wrap"><img src="${safeAttr(a.image)}" alt="${escapeHTML(a.title)}" loading="eager" onerror="this.src='${placeholderImage(a.title)}'"></div><div class="card-body"><span>${escapeHTML(titleCase(a.section || a.topic))}</span><h1>${escapeHTML(a.title)}</h1><p>${escapeHTML(a.summary || a.description)}</p><small>${escapeHTML(a.sourceName || a.source || "Global Intel")} • ${formatDate(a.publishedAt)}</small></div></a></article>`;
}
function smallCard(a) { return a ? `<div class="stack-card">${card(a)}</div>` : loadingCard(); }
function card(a) {
  if (!a) return loadingCard();
  return `<article class="news-card"><a href="./article.html?id=${a.id}"><div class="card-image-wrap"><img src="${safeAttr(a.image)}" alt="${escapeHTML(a.title)}" loading="lazy" onerror="this.src='${placeholderImage(a.title)}'"></div><div class="card-body"><span class="section-tag">${escapeHTML(titleCase(a.section || a.topic || "News"))}</span><h3>${escapeHTML(a.title)}</h3><p>${escapeHTML(a.summary || a.description || "")}</p><small>${escapeHTML(a.sourceName || a.source || "Global Intel")} • ${formatDate(a.publishedAt)}</small></div></a></article>`;
}
...
function normalizeArticle(item) {
  ...
  return {
    ...
    image: item.image || item.urlToImage || item.thumbnail || placeholderImage(title),
...
function placeholderImage(seed) { return `https://picsum.photos/seed/${encodeURIComponent(seed || "global-intel")}/900/560`; }
