...
export async function loadFeeds(feeds, topic='usa') {
  const results = await Promise.allSettled(feeds.slice(0,25).map(url => loadFeed(url)));
  let articles = [], errors = [];
  ...
  articles = dedupe(articles)
    ...
    .filter(a => topic === 'usa' || topic === 'latest' ? true : articleMatchesTopic(a, topic))
...
async function loadFeed(url) {
  const r = await fetch(url, { headers:{ 'user-agent':'GlobalIntelTimes/7.0' }});
  if (!r.ok) throw new Error(`${url} failed ${r.status}`);
  return parseRSS(await r.text(), url);
}

function parseRSS(xml, sourceUrl) {
  const items = [...xml.matchAll(/<item[\s\S]*?<\/item>|<entry[\s\S]*?<\/entry>/gi)].slice(0,30);
  return items.map(m => {
    const block = m[0];
...
export function articleMatchesTopic(a, topic){ const t=String(topic||'').toLowerCase().replace(/-/g,' '); const text=`${a.title} ${a.description} ${a.section} ${a.topic}`.toLowerCase().replace(/-/g,' '); return !t||t==='usa'||t==='latest'||text.includes(t)||detectSection(text)===topic; }
