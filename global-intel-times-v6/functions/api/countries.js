...
const COUNTRIES = ['United States','China','Russia','Ukraine','India','United Kingdom','Israel','Iran','Japan','Germany','France','Brazil','Canada','Australia','Mexico','South Korea','Saudi Arabia','Turkey','South Africa','Indonesia'];
...
async function countryNews(country) {
  try {
    ...
    const articles = (d.articles || []).map(a => ({title:a.title,url:a.url,source:a.domain || a.sourceCountry || 'GDELT',time:a.seendate,image:a.socialimage || '',description:a.snippet || ''}));
  ...
  } catch(e) { return json({country, riskScore:0, risk:'Unknown', articles:[], error:e.message}); }
}
