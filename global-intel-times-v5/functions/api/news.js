...
export async function onRequestGet({ request, env }) {
  ...
  for (const endpoint of ['news-a','news-b','news-c','news-d']) {
    try {
      const r = await fetch(`${origin}/api/${endpoint}?topic=${encodeURIComponent(topic)}`);
  ...
  if (env.NEWS_API_KEY) {
    try {
      ...
      articles.push(...(d.articles || []).map(a => ({
        title:a.title, description:a.description, content:a.content || a.description, author:a.author || a.source?.name || 'NewsAPI',
        image:a.urlToImage || '', publishedAt:a.publishedAt, url:a.url, section:'latest', topic:'latest'
      })));
    } catch(e) { errors.push('NewsAPI: '+e.message); }
  }

  if (env.GNEWS_API_KEY && articles.length < 60) {
    try {
      ...
      articles.push(...(d.articles || []).map(a => ({
        title:a.title, description:a.description, content:a.content || a.description, author:a.source?.name || 'GNews',
        image:a.image || '', publishedAt:a.publishedAt, url:a.url, section:'latest', topic:'latest'
      })));
  ...
  return json({topic,articles:articles.length?articles:fallbackArticles(topic),error:errors.length?errors.slice(0,8).join(' | '):undefined});
}
