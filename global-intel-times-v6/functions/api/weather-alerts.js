...
export async function onRequestGet({env}) {
  if (!env.WEATHER_API_KEY) return json({source:'OpenWeather', count:1, alerts:[{city:'New York', level:'Info', title:'Weather API key not configured', description:'Add WEATHER_API_KEY to enable live alerts.'}], error:'WEATHER_API_KEY missing'});
  let alerts = [], errors = [];
  await Promise.allSettled(CITIES.map(async city => {
    try {
      ...
      const temp=d.main?.temp, wind=d.wind?.speed, condition=d.weather?.[0]?.description || 'Weather update';
      let level='Low'; if(temp>=38 || wind>=18) level='High'; else if(temp>=32 || wind>=11) level='Medium';
      alerts.push({city, level, title:condition, temp, wind, humidity:d.main?.humidity, time:new Date().toISOString()});
    } catch(e) { errors.push(`${city}: ${e.message}`); }
  }));
